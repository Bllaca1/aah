...
const NOTIFICATION_ICONS: { [key in NotificationType]: React.ReactNode } = {
  ...
  [NotificationType.GENERIC]: <Info className="h-5 w-5 text-purple-400" />,
...
const NotificationItem: React.FC<NotificationItemProps> = ({ notification, onClose }) => {
    ...
    const content = (
        <div 
            ...
            className="flex items-start p-3 hover:bg-gray-700/50 transition-colors duration-200 cursor-pointer border-b border-gray-700 last:border-b-0"
        >
            <div className="flex-shrink-0 mr-4 mt-1">
                {notification.sender ? (
                    <img src={notification.sender.avatarUrl} alt={notification.sender.username} className="h-8 w-8 rounded-full" />
                ) : (
                    <div className="h-8 w-8 flex items-center justify-center bg-gray-600 rounded-full">
            ...
            <div className="flex-grow">
                 <p className="text-sm text-gray-300">
                    {notification.sender && <span className="font-bold text-white">{notification.sender.username}</span>} {notification.message}
                </p>
